...
def test_ikev2_parser_emits_observed_fields():
    ...
    observed = {(finding.source_field, finding.raw_value) for finding in findings if finding.derivation == "observed"}
...
def test_parser_never_labels_esp_cipher_or_mode_as_observed():
    ...
    observed_fields = {finding.source_field for finding in findings if finding.derivation == "observed"}
